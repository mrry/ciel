inputs_url_ref = ref("http://localhost/~avsm/nbody.txt");
coords_url_ref = ref("http://localhost/~avsm/coords.txt");
...
function walkTree(root_ref, tree_refs) {
   ...
   if (last_tree["count"] == 0) {
      return exec("stdinout", {"inputs":[], "command_line": ["echo"]}, 1);
   }
   if (last_tree["count"] == 1) {
      return exec("stdinout", {"inputs":[tree_refs[tree_length - 1]], "command_line": ["python", "-c", "import json; import sys; print json.dumps(json.load(sys.stdin)[\"centroid\"])"]}, 1);
   }
   new_tree_refs = [];

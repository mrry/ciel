...
def stream_producer(chunk_size, chunks_to_produce, use_direct_pipes):
    ...
    with skypy.open_output(skypy.extra_outputs[0], may_pipe=use_direct_pipes, single_consumer=True) as file_out:
...
def stream_link(chunk_size, input_ref, use_direct_pipes):
    ...
    with skypy.open_output(skypy.extra_outputs[0], may_pipe=use_direct_pipes, single_consumer=True) as out_file:

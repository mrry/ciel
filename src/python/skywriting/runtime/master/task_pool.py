...
class TaskPool(plugins.SimplePlugin):
    ...
    def spawn_child_tasks(self, parent_task, spawned_task_descriptors):
        ...
        # TODO: different handling for replayed tasks.
        for child in spawned_task_descriptors:
            try:
                spawned_task_id = uuid.UUID(hex=child['task_id'])
                expected_outputs = [uuid.UUID(hex=x) for x in child['expected_outputs']]
    ...
    def commit_task(self, task_id, commit_payload):
        ...
        try:
            replay_uuid_list = [uuid.UUID(hex=x) for x in commit_payload['replay_uuids']]
            print 'Replay UUIDs for task %s: %s' % (repr(task_id), repr(replay_uuid_list))

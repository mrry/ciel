...
MASTER=${MASTER_HOST:-http://$HOST:8000}
...
${PYTHON} ${BASE}/src/python/skywriting/__init__.py --role worker --master ${MASTER} --port $WORKER_PORT --staticbase $BASE/src/js/skyweb_worker/ --lighttpd-conf $BASE/src/python/skywriting/runtime/lighttpd.conf -b $BASE/$REL_BLOCK_LOCATION
